...
import {createOrEditCategory, deleteCategory, getAllCategories} from "../service/CategoryService";
...
class CategoryInfo extends Component {
    ...
    submitForm() {
        ...
        createOrEditCategory(catCode, catName, editCatId, parent).then(() => {
            ...
            Alert.error((err && err.message) || 'Не удалось сохранить категорию, попробуйте еще раз');
        })
        this.closeModal()
    }
    ...
    loadCategories() {
        getAllCategories().then(result =>
            this.setState({
                ...
                allCategories: result.content.map(({id, name}) => ({id: id, value: id, label: name}))
            })).catch(() =>
            this.setState({
                categories: [],
                allCategories: []
        ...
        return (
            <Segment>
                            ...
                            <Table.Body>
                                {
                                    categories.map((cat, index) => (
                                        <Table.Row key={index}>
                                            ...
                                            <Table.Cell>
                                                <Button onClick={() => {
                                                    if (window.confirm("Are you sure?")) this.onDeleteCat(cat.id)
                ...
                <Modal
                    ...
                    open={showModal}
                    size={'small'}
                    trigger={<Button primary style={{backgroundColor: '#175e6b'}}>Добавить категорию</Button>}
                >
                    <Modal.Content>
                        <Modal.Description>
